...
module "talos_cluster" {
  source = "git::https://github.com/hovorka-labs/iac-modules.git//terraform/modules/talos?ref=talos-v1.0.8"
...
- **`zone` defaults to the node's own map key**, but can be overridden. On Proxmox, set it to the real Proxmox node name (not this node's identity in the `nodes` map) if you're running Proxmox CSI or CCM - both call the Proxmox API using `topology.kubernetes.io/zone` directly as a node name, and fail (Proxmox CSI's `GetCapacity` in particular, with an opaque "unexpected end of JSON input") if it's set to anything else.
